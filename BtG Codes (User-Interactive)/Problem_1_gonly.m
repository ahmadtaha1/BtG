function Problem_1_gonly(N,Nb,Ng,Nc,b_s,g_s,base,z,count_tg,count_tb,xb0,xg0,opt_ub)
% Author: Ankur Pipri
...
b2{1,1}=xg0;
for i=2:1:g_s
    b2{i,1}=(sys_dg.B_wg*sys_g.w_g(:,(count_tg-1+i))) + sys_dg.B_gub*(-1/(10^6*base))*opt_ub;
end
b2=cell2mat(b2);
Beq{1,1}=b2;
...
f3=cost_n.lincost_usp;
P1.f=sparse([zeros((2*N*g_s),1);f2;f3]);
...
Bneq{1,1}=(limit_n.lf + sys_g.PTDF*sys_g.Pi*sys_g.umisc(:,count_tg) + sys_g.PTDF*sys_g.Pi*(-1/(10^6*base))*opt_ub + sys_g.PTDF*sys_g.bl(:,count_tg));
Bneq{2,1}=(limit_n.lf - sys_g.PTDF*sys_g.Pi*sys_g.umisc(:,count_tg) - sys_g.PTDF*sys_g.Pi*(-1/(10^6*base))*opt_ub - sys_g.PTDF*sys_g.bl(:,count_tg));
